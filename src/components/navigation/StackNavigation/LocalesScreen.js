...
const LocalesScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const query = 'tacos,garnachas,gorditas,elotes,tortas';
                const region = 'mx:qro';
                const maxResults = 10;
                ...
                const detailedLocales = await Promise.all(
        ...
        <ScrollView contentContainerStyle={styles.container}>
            {locales.map((local) => (
                <TouchableOpacity
                    key={local.id}
                    onPress={() =>
                        navigation.navigate('PlaceDetail', { place: local })
                    }
                >
                    <Card key={local.id} style={styles.card}>
                        <Card.Content style={styles.cardContent}>
                            ...
                            <View style={styles.textContainer}>
                                ...
                                <Text style={styles.detail}>
                                    Reseñas: {local.reviewCount} | Dirección:{' '}
                                    {local.detail}
                                </Text>
                            </View>
                            <List.Icon
